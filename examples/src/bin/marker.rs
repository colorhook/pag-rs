...
fn main() {
  ...
  let marker_file = PAGFile::from_file(assets_dir.join("marker.pag").to_str().unwrap());
  ...
  let audio_file = PAGFile::from_file(assets_dir.join("AudioMarker.pag").to_str().unwrap());
